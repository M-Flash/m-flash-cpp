...
template <class V, class E, class IdType>
  class DegreeOperator : public MAlgorithm<V, V, E, IdType>{
	...
	public:
	  template <class V, class E, class IdType>
	  static inline void run(Matrix<E, IdType> &matrix, PrimitiveVector<V> &destination, int iterations){
      ...
      PrimitiveVector<V> degree_vec(degree_file, matrix.size(), matrix.get_elements_by_block());

      PrimitiveVector<V> *current_vec = &destination;
      PrimitiveVector<V> *next_vec = new PrimitiveVector<V> (next_file, matrix.size(), matrix.get_elements_by_block());
      PrimitiveVector<V>* tmp;
      ...
      if(!exist_file(degree_file)){
          //matrix.operate(degree_operator, degree_vec);
      }
