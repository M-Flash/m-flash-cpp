...
public:
	SplitterBufferWithBlockCounting(std::string graph, int64 edge_data_size, int64 buffer_size, int64 ids_by_partitions, bool in_split = true, std::string file_prefix = "", int64 partitions = 0 );
	std::vector<int64> getCounters();
};

template <class IdType> inline
SplitterBufferWithBlockCounting<IdType>::SplitterBufferWithBlockCounting(std::string graph, int64 edge_data_size, int64 buffer_size, int64 ids_by_partition, bool in_split, std::string file_prefix, int64 partitions)
: SplitterBuffer<IdType>(graph, edge_data_size, buffer_size, ids_by_partition, in_split, file_prefix,  partitions){
...
template <class IdType> inline
IdType SplitterBufferWithBlockCounting<IdType>::countEdge(IdType in_id, IdType out_id){
