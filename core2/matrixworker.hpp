...
namespace mflash{
  ...
  class MatrixWorker;
	...
	class MatrixWorker{
      ...
      BlockProperties** block_preprocessing(int block_count);
			...
			MatrixWorker(Matrix<E> &matrix) : MatrixWorker(matrix, true){}

			Matrix<E> get_matrix(){return *matrix;}
  ...
  template <class E>
  inline void MatrixWorker<E>::set_default_destination_field(AbstractVector *vector){
...
*/
  ...
  inline void MatrixWorker<E>::load_fields(FieldType type, int64 offset){
    ...
    if(FieldType::SOURCE == type){
        ...
        if (default_source != 0){
            Array<VSource> *source_wrapped = dynamic_cast< Array<VSource>* >(source_pointer);
            source_wrapped->set_limit(default_source->load_region(offset, block_size, source_pointer->address()));
        ...
        if (default_destination != 0){
            ...
            destination_wrapped->set_limit(default_source->load_region(offset, block_size, destination_pointer->address()));
            destination_wrapped->set_offset(offset);
        }
    }

    for ( int i = pos ; i < size; i ++ ){
        array_pointers[i]->set_limit(vector_pointers[i]->load_region(offset, block_size, array_pointers[i]->address()));
  ...
  inline void MatrixWorker<E>::store_fields(int64 offset){
    if (default_destination != 0){
        default_destination->store_region(offset, block_size(), destination_pointer->address());
    }
  }
  ...
  void MatrixWorker<E>::operate(MAlgorithm<VSource,VDestination, E> &algorithm){
    ...
    //block iteration
    BlockIterator iterator(matrix->get_file(), blocks, matrix->is_transpose()?1:0);
    ...
    InitializeOperator<VSource, VDestination,E> initialize_operator (&algorithm, this);
    ...
    while(iterator.has_next()){
      ...
      source_limit =  min(source_offset + block_size, elements-source_offset) -1;
      ...
      if(last_col != block.get_col() && BlockType::M_FLASH == properties->type){
      ...
      AbstractProcessor *processor = new EdgeProcessor<VSource, VDestination, E>(*this, algorithm);
...
*/
  ...
  BlockProperties** MatrixWorker<E>::block_preprocessing(int block_count){
    ...
    int64 edge_size =  2 * (matrix->get_element_id_size() == ElementIdSize::SIMPLE? sizeof(int): sizeof(int64)) + sizeof(E);
    ...
    BlockIterator iterator(matrix->get_file(), block_count, matrix->is_transpose()?0:1);
  ...
  class InitializeOperator : public UnaryOperator<VDestination>{
    ...
    public:
        InitializeOperator(MAlgorithm<VSource, VDestination,E> *algorithm, MatrixWorker<E> *worker){
            this->algorithm = algorithm;
            this->worker = worker;
        }
  ...
  class AppyOperator: public UnaryOperator<VDestination>{
    ...
    public:
      AppyOperator(MAlgorithm<VSource, VDestination,E> *algorithm, MatrixWorker<E> *worker){
          this->algorithm = algorithm;
          this->worker = worker;
      }
