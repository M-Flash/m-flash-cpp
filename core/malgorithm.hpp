...
namespace mflash{
	...
	class MAlgorithm{
	     ...
	     */
			virtual void initialize(MatrixWorker<V, E> &worker, Element<V> &out_element) = 0;
       ...
       */
			virtual void gather(MatrixWorker<V, E> &worker, Element<V> &source, Element<V> &destination, E &edge_data) = 0;

			/**
			 * Called to combine accumulators between threads.
			 */
			virtual void sum(Element<V> &accumulator1, Element<V> &accumulator2, Element<V> &out_accumulator) = 0;

			/**
			 * Called after combine the accumulators for each vertex to update the vertex value.
			 */
			virtual void apply(MatrixWorker<V, E> &worker, Element<V> &out_element) = 0;
       ...
       */
			 ...
			 */
			virtual void before_iteration(int iteration, MatrixWorker<V,E> &worker){
			 ...
			 */
      virtual void after_iteration(int iteration, MatrixWorker<V,E> &worker){
      }
